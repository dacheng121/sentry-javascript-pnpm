...
run('pnpm rollup -c rollup.npm.config.js');
...
// Because we import our polyfills from `@monitor-web/utils/cjs/buildPolyfills` and `@monitor-web/utils/esm/buildPolyfills` rather
// than straight from `@monitor-web/utils` (so as to avoid having them in the package's public API), when tests run, they'll
